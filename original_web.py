...
def hello(name="quake0day", id=1):
    ...
    processingCheckDbConn = create_connection(processingCheckDb)
    ...
    processingCheckDbCur = processingCheckDbConn.cursor()
    processingCheckDbCur.execute("SELECT isProcessing FROM users WHERE user = ?", (name,))
    isProcessingStatusForUser = processingCheckDbCur.fetchone()

    if isProcessingStatusForUser is not None:
